"use client";

import CircularProgress from "@mui/material/CircularProgress";
import { useGoogleLogin } from "@react-oauth/google";

interface GoogleButtonProps {
  onSuccess: (code: string) => void;
  onError?: (error?: any) => void;
  text?: string;
  loading?: boolean;
  className?: string;
  scope?: string;
}

export default function GoogleButton({
  onSuccess,
  onError,
  text = "Connect in with Google",
  loading = false,
  className,
  scope = "openid email profile https://www.googleapis.com/auth/gmail.readonly https://www.googleapis.com/auth/admin.directory.user.readonly https://www.googleapis.com/auth/drive.readonly https://www.googleapis.com/auth/calendar.readonly https://www.googleapis.com/auth/contacts.readonly",
}: GoogleButtonProps) {
  const googleLogin = useGoogleLogin({
    flow: "auth-code",
    scope: scope || "openid email profile",
    onSuccess: (codeResponse) => {
      onSuccess(codeResponse.code);
    },
    onError: (error) => {
      if (onError) onError(error);
    },
  });

  return (
    <button
      onClick={() => googleLogin()}
      disabled={loading}
      className={
        className ||
        "w-full flex items-center justify-center gap-3 px-6 py-4 border border-(--border) rounded-md bg-(--bg-secondary) hover:bg-(--bg-active) text-(--text-primary) font-bold transition-all duration-300 cursor-pointer disabled:opacity-75 disabled:cursor-not-allowed"
      }
    >
      {/* Google SVG */}
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 48 48"
        className="w-7 h-7 sm:w-6 sm:h-6"
      >
        <path
          fill="#FFC107"
          d="M43.6 20.5H42V20H24v8h11.3C33.6 32.7 29.2 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3 0 5.7 1.1 7.8 3l5.7-5.7C34.1 6.1 29.4 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.3-.4-3.5z"
        />
        <path
          fill="#FF3D00"
          d="M6.3 14.7l6.6 4.8C14.7 16 19 12 24 12c3 0 5.7 1.1 7.8 3l5.7-5.7C34.1 6.1 29.4 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"
        />
        <path
          fill="#4CAF50"
          d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2C29.2 35.5 26.7 36 24 36c-5.2 0-9.6-3.3-11.3-8l-6.5 5C9.5 39.5 16.2 44 24 44z"
        />
        <path
          fill="#1976D2"
          d="M43.6 20.5H42V20H24v8h11.3c-1.1 3.1-3.3 5.5-6.1 7.1l6.2 5.2C39.7 36.3 44 30.7 44 24c0-1.3-.1-2.3-.4-3.5z"
        />
      </svg>

      {loading ? (
        <CircularProgress size={16} color="inherit" />
      ) : (
        <span>{text}</span>
      )}
    </button>
  );
}
